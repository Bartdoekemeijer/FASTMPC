...
while Wp.k<Wp.N/(2*Wp.h)
    ...
    sr.e(Wp.k)         = gp.Pnref(Wp.k) - sum(ymes(:,2));   % wind farm error      
...
for ll=1:Wp.Nt
    ...
    PALM.Power(ll,:) = M(1:2:end,8)';
    PALM.phi(ll,:)   = M(1:2:end,6)';
    PALM.time(ll,:)  = M(1:2:end,1)';
    PALM.CT(ll,:)    = M(1:2:end,4)';
    PALM.Ur(ll,:)    = M(1:2:end,2)';
    PALM.aif(ll,:)   = M(1:2:end,5)';
end
...
clear temp M ll filename cmdout status xmes umes vmes ymes uc kk str MaxTime timer tline fid 
